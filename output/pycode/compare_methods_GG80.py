# -*- coding: utf-8 -*-
"""
对比分析脚本：GG80 SupportCut方法 vs RelativeBalance求解器直接求解
"""
...
# 读取数据
supportcut_df = pd.read_excel('D:/NJU/dro理论学习/机会约束区域划分/TDP-pre/TDP-pre/output/csv/csv_plot/GG50+GG80+SupportCut.xlsx')
solver_df = pd.read_excel('D:/NJU/dro理论学习/机会约束区域划分/TDP-pre/TDP-pre/output/csv/csv_plot/GG50+GG80+RelativeBalance.xlsx')

print("SupportCut数据行数:", len(supportcut_df))
print("RelativeBalance数据行数:", len(solver_df))
...
# 计算相对gap百分比: (Cut - Solver) / Solver * 100%
objective_gap_percent = (supportcut_common['Objective'].values - solver_common['Objective'].values) / solver_common['Objective'].values * 100
...
# fig.suptitle('GG80实验：SupportCut方法 vs 求解器直接求解 对比分析', fontsize=16, fontweight='bold')

# 子图1: 目标函数相对Gap百分比 (折线图)
ax1 = axes[0]
ax1.plot(experiment_ids, objective_gap_percent, 'b-o', linewidth=1.5, markersize=3, label='相对Gap (Cut-Solver)/Solver')
ax1.axhline(y=0, color='r', linestyle='--', alpha=0.5, label='零线')
ax1.set_xlabel('实验序号', fontsize=11)
ax1.set_ylabel('相对Gap (%)', fontsize=11)
ax1.set_title('目标函数相对Gap对比 (Cut方法 vs Solver方法)', fontsize=12)
ax1.legend(loc='best')
ax1.grid(True, alpha=0.3)
ax1.set_xlim(0.5, len(experiment_ids) + 0.5)

# 添加统计信息
mean_gap = np.mean(objective_gap_percent)
max_gap = np.max(objective_gap_percent)
min_gap = np.min(objective_gap_percent)
ax1.text(0.02, 0.95, f'平均Gap: {mean_gap:.4f}%\n最大Gap: {max_gap:.4f}%\n最小Gap: {min_gap:.4f}%', 
         transform=ax1.transAxes, fontsize=10, verticalalignment='top', 
         bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))

# 子图2: Out of Sample对比 (折线图)
ax2 = axes[1]
ax2.plot(experiment_ids, oos_cut, 'b-o', linewidth=1.5, markersize=3, label='SupportCut', alpha=0.8)
ax2.plot(experiment_ids, oos_solver, 'r-s', linewidth=1.5, markersize=3, label='求解器直接求解', alpha=0.8)
...
# 根据数据范围动态设置Y轴
min_oos = min(np.min(oos_cut), np.min(oos_solver))
ax2.set_ylim(max(0.9, min_oos - 0.02), 1.01)
...
# 子图3: 计算时间对比 (双折线图)
ax3 = axes[2]
ax3.plot(experiment_ids, time_cut, 'b-o', linewidth=1.5, markersize=3, label='SupportCut', alpha=0.8)
ax3.plot(experiment_ids, time_solver, 'r-s', linewidth=1.5, markersize=3, label='求解器直接求解', alpha=0.8)
...
plt.tight_layout()
plt.savefig('D:/NJU/dro理论学习/机会约束区域划分/TDP-pre/TDP-pre/output/pycode/comparison_analysis_GG80.png', dpi=150, bbox_inches='tight')
plt.savefig('D:/NJU/dro理论学习/机会约束区域划分/TDP-pre/TDP-pre/output/pycode/comparison_analysis_GG80.pdf', bbox_inches='tight')
print("\n图表已保存为 comparison_analysis_GG80.png 和 comparison_analysis_GG80.pdf")

# 输出详细统计
print("\n" + "="*60)
print("详细统计信息")
print("="*60)
print(f"\n1. 目标函数相对Gap (Cut - Solver) / Solver * 100%:")
print(f"   平均值: {np.mean(objective_gap_percent):.4f}%")
print(f"   标准差: {np.std(objective_gap_percent):.4f}%")
print(f"   最大值: {np.max(objective_gap_percent):.4f}%")
print(f"   最小值: {np.min(objective_gap_percent):.4f}%")
